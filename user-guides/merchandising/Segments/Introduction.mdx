...
Segmentation categorizes customers into groups based on shopping history, geographic location, device type, and other attributes. This enables targeted marketing and merchandising strategies. Unbxd provides a platform to create and manage segments, allowing the configuration of predefined and custom attributes such as location, device, and user type.
...
| Segment's attributes | Groups shoppers by location, device, and visit type |
...
<Info>
  ...
  2. All of them can be rearranged in order of importance by drag and drop operations on the
     console. Learn more about [Default attributes and Custom]() attribute (link Attributes MDX file here)
</Info>
...
Custom attributes can be selectively deleted as long as they are not used in any active\
segment.

<Info>
  **Active Segment ** :If a segment is being used by any active or upcoming

  campaigns in search or browse.
</Info>

However, Unbxd default attributes are permanent and cannot be removed from the
...
Click on the “add new segment” button and select criteria based on your requirement explained next.
