...
2. **Vector Search** must be enabled on your UNBXD panel. To do this, click on the **"Contact support**" link, which will auto create a ticket for enabling this feature at the product end for approval.\
   \
   This could be enabled by clicking on the toggle button, if it’s not yet activated. Once the feature is enabled, the Customer is set to use the Measurement search for the search-related queries.
